...
class lr_address_parser():

    def __init__(self, user='APIS', pwd='APIS', server='10.0.0.182:1521', db='apispdb'):
        self.user = user
        self.pwd = pwd
        self.server = server
        self.db = db
        self.conn = Oracle(user, pwd, server, db)

    def prop_data_load(self):
        conn = self.conn
    ...
    def addr_parse(self, namestr, cur_r, cur_m, cur_d, cur_e, cur_p, cur_b, cur_eb_p):
        v_region_id = ''
        v_str_region = ''
        v_str_mjdist = ''
        v_mjdist_code = ''
        ...
        if v_str_lot_full != '':
            while re.search(
                    '(?P<raw_lotext>((S\.|SECTION\s)[A-N]|(S\.?S\.\s?|SUB-SECTION\s|SUB-SEC\.)\d+|(R\.?P\.|REMAINING\sPORTION\s)|(EX\.|EXTENSION))\s?)',
                    unparse_namestr) is not None:
                res = re.search(
                    '(?P<raw_lotext>((S\.|SECTION\s)[A-N]|(S\.?S\.\s?|SUB-SECTION\s|SUB-SEC\.)\d+|(R\.?P\.|REMAINING\sPORTION\s)|(EX\.|EXTENSION))\s?)',
                    unparse_namestr)
        ...
        if v_phase_id != '':
            ...
            if len(b_cur_pe) == 1:
                ...
                for item in b_cur_pe:
                    bname = ' ' + item[3]
                    if unparse_namestr.find(bname) >= 0 or (
                            bname.find(v_str_bldg_raw_map) >= 0 and v_str_bldg_raw != ''):
        ...
        # STREET
        res = re.search('(?P<raw_street>(NOS?\.)?\s?\d+(\s?-\s?\d+)?\s(\(.*\)\s)?[^\d]*(STREET|ROAD|LANE))',
                        unparse_namestr)
        if res is not None:
            v_str_street_full = res.groupdict()['raw_street']
            unparse_namestr = unparse_namestr.replace(v_str_street_full, '')
        else:
            res = re.search('(?P<raw_street_pre>(NO(S)?\.)?\s?\d+[a-gA-G]?(-\s?\d+)?\s?(\((\w|\s)*\)\s?)?[^(\d|/)]*)',
                            unparse_namestr)
            if res is not None:
    ...
    def unit_exact_match(self, v_est_id, v_phase_id, v_bldg_id, v_str_floor_net, v_str_unit_net):
        conn = self.conn
    ...
    def parser_output(self, namestr):
        #conn = self.conn
        cur_r, cur_m, cur_d, cur_s, cur_e, cur_p, cur_b, cur_eb_p = self.prop_data_load()
        ...
        v_str_floor_full, v_str_floor_net, v_str_unit_full, v_str_unit_net, v_has_carpark, v_has_roof, \
        unparse_namestr = self.addr_parse(namestr, cur_r, cur_m, cur_d, cur_e, cur_p, cur_b, cur_eb_p)

        cur_u, unit_num, map_status, unit_id = self.unit_exact_match(v_est_id, v_phase_id, v_bldg_id, v_str_floor_net, v_str_unit_net)

        return v_region_id, v_mjdist_code, v_dist_code, v_est_id, v_phase_id, v_bldg_id, v_has_carpark, v_has_roof, unit_id, unit_num, map_status
...
if __name__ == '__main__':
    ...
    username = 'APIS'
    password = 'APIS'
    server = '10.0.0.182:1521'
    db = 'apispdb'

    lr_address_parser = lr_address_parser(username, password, server, db)

    v_region_id, v_mjdist_code, v_dist_code, v_est_id, v_phase_id, v_bldg_id, v_has_carpark, v_has_roof, unit_id, unit_num, \
    map_status = lr_address_parser.parser_output(namestr)
